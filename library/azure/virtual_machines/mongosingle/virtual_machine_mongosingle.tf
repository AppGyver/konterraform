resource "azurerm_virtual_machine" "kontena_mongosingle" {
  ...
  vm_size               = "${var.azure_virtual_machine_mongosingle_vm_size}"
  ...
  storage_os_disk {
    name          = "kontena-mongosingle-${count.index}"
    vhd_uri       = "${azurerm_storage_account.kontena.primary_blob_endpoint}${azurerm_storage_container.kontena.name}/kontena-mongosingle-${count.index}.vhd"
    caching       = "ReadWrite"
    create_option = "FromImage"
  }

  delete_os_disk_on_termination = "${var.azure_virtual_machine_mongosingle_delete_os_disk_on_termination}"

  os_profile {
    computer_name  = "${var.name}-kontena-mongosingle-${count.index}"
    admin_username = "core"
    admin_password = "ThisShouldBeDisabledButIsRequired${count.index}${random_id.kontena_password.hex}"

    custom_data = "${base64encode(element(data.template_file.coreos_cloudconfig_mongosingle.*.rendered, count.index))}"
  }
